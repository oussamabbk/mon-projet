...
{    
    char nomn[20],prenomn[20],emailn[20],specialiten[20],page_facebookn[8],numeron[20];
   
   
    FILE *f , *tmp;
    f=fopen("/home/lenovo/Projets/projet3/src/profile_coach.txt","r");
    tmp=fopen("/home/lenovo/Projets/projet3/src/profile_coach.tmp","a+");
    while(fscanf(f,"%s %s %s %s %s %s\n",nomn,prenomn,emailn,specialiten,page_facebookn,numeron)!=EOF){
        if(!strcmp(nom,nomn) && !strcmp(prenom,prenomn) ) {fprintf(tmp,"%s %s %s %s %s %s\n",nom,prenom,email,specialite,page_facebook,numero);}
else {fprintf(tmp,"%s %s %s %s %s %s\n",nomn,prenomn,emailn,specialiten,page_facebookn,numeron);
}
fclose(f);
fclose(tmp);
rename("/home/lenovo/Projets/projet3/src/profile_coach.tmp","/home/lenovo/Projets/projet3/src/profile_coach.txt");
}
} 
void ajouter_poid_ideal (char poid[])
{
	FILE *f;
	f=fopen("/home/lenovo/Projets/projet3/src/poid_ideal.txt","a+");
	if(f!=NULL){
		fprintf(f,"%s \n",poid);
	}
	fclose(f);
